...
export default function TodoModal({
  ...
  // 메모
  const [memos, setMemos] = useState<TodoMemo[]>([]);
  ...
  const myMemo =
    isEdit && user ? memos.find((m) => m.userId === user.userId) : undefined;
  ...
  const handleMemoSave = async () => {
    if (!todo || !memoInput.trim()) return;
    setMemoSaving(true);
    try {
      await todoApi.saveMemo(todo.id, memoInput.trim());
  ...
  const handleMemoDelete = async () => {
    ...
    try {
      await todoApi.deleteMemo(todo.id);
      setMemoEditing(false);
      setMemoInput("");
      loadMemos();
    } finally {
  ...
  const formatMemoTime = (memo: TodoMemo) => {
    const isUpdated = memo.updatedAt !== memo.createdAt;
    const dt = new Date(isUpdated ? memo.updatedAt : memo.createdAt);
    const mm = String(dt.getMonth() + 1).padStart(2, "0");
    const dd = String(dt.getDate()).padStart(2, "0");
    const hh = String(dt.getHours()).padStart(2, "0");
    const min = String(dt.getMinutes()).padStart(2, "0");
    return `${mm}.${dd} ${hh}:${min}${isUpdated ? " (수정됨)" : ""}`;
  };

  // 그룹 선택
  const [groups, setGroups] = useState<MyGroupSummary[]>([]);
  const [selectedGroupId, setSelectedGroupId] = useState<number | undefined>(
    todo?.groupId ?? undefined,
  );
  ...
  const isCreator = !!(
    ...
    todo &&
    user &&
    todo.creatorUserId === user.userId
  );
  const isGroupLeader = !!(
    isEdit &&
    todo?.groupId &&
    groups.find((g) => g.groupId === todo.groupId)?.myRole === "LEADER"
  );
  const canDelete = isCreator || isGroupLeader;

  const deleteConfirmTitle =
    isEdit && todo?.groupId ? "그룹 Todo를 삭제할까요?" : "Todo를 삭제할까요?";

  const deleteConfirmDescription =
    isEdit && todo?.groupId
      ? "삭제하면 이 Todo가 그룹원 화면에서도 함께 사라져요."
      : "삭제한 Todo는 복구하기 어려워요.";

  const openDeleteConfirm = () => {
    if (!todo || deleting || loading) return;
  ...
  return (
    <>
      ...
      {showDeleteConfirm && (
        ...
        >
          <div
            className="absolute left-1/2 top-1/2 w-[calc(100%-32px)] max-w-[360px] -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white shadow-2xl"
          ...
          >
            ...
            <div className="px-5 pb-5 flex gap-2">
              <button
                ...
                disabled={deleting}
                className="flex-1 py-3 rounded-xl text-sm font-semibold border border-gray-200 text-gray-500 bg-white active:opacity-70 disabled:cursor-not-allowed"
              >
                취소
              </button>
              ...
              <button
                ...
                disabled={deleting}
                className="flex-1 py-3 rounded-xl text-sm font-semibold border bg-white active:opacity-70 disabled:cursor-not-allowed"
                style={{
                  borderColor: deleting ? "#D1D5DB" : "#111111",
                  color: deleting ? "#9CA3AF" : "#111111",
                }}
              >
                {deleting ? "삭제 중..." : "삭제"}
              </button>
        ...
        >
          ...
          >
            <div className="px-5 py-3 flex flex-col gap-3">
              ...
              <textarea
                className="w-full h-[88px] max-h-40 border border-gray-200 rounded-xl px-4 py-3 text-sm leading-5 align-top outline-none focus:border-[#E85D2F] transition-colors resize-none overflow-y-auto"
                placeholder="내용 (선택)"
                value={content}
                onChange={(e) => setContent(e.target.value)}
              />

              {/* 그룹 표시: 수정 모드에서 이미 그룹이 있으면 읽기 전용 */}
              {isEdit && todo?.groupId ? (
                ...
                <div ref={groupDropdownRef} className="relative">
                  <button
                    ...
                    style={{
                      borderColor: selectedGroupId ? "#4AAFCC" : "#E5E7EB",
                    }}
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span
                        className="truncate"
                        style={{
                          color: selectedGroupId ? "#4AAFCC" : "#6B7280",
                        }}
                      >
                        {selectedGroup
                ...
                <div className="flex flex-col gap-2 pt-1">
                  ...
                  {memoEditing && (
                    <div className="flex flex-col gap-2">
                      <textarea
                        className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-[#4AAFCC] transition-colors resize-none"
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 py-2 rounded-xl text-sm font-semibold text-white transition-opacity active:opacity-80 disabled:cursor-not-allowed"
                          style={{
                            background:
                              memoSaving || !memoInput.trim()
                                ? "#CCCCCC"
                                : "#4AAFCC",
                          }}
                        >
                        ...
                        <button
                          ...
                          onClick={() => {
                            setMemoEditing(false);
                            setMemoInput(myMemo?.content ?? "");
                          }}
                          className="flex-1 py-2 rounded-xl text-sm font-semibold border border-gray-200 text-gray-500 active:opacity-70"
                        >
                          취소
                        </button>
                        ...
                        {myMemo && (
                          <button
                            ...
                            disabled={memoDeleting}
                            className="px-4 py-2 rounded-xl text-sm font-semibold border text-red-400 border-red-200 active:opacity-70 disabled:cursor-not-allowed"
                          >
                            {memoDeleting ? "삭제 중..." : "삭제"}
                          </button>
                        )}
                      </div>
                    </div>
                  ...
                  {!memoEditing &&
                    memos.map((memo) => {
                      const isMe = user?.userId === memo.userId;
                      ...
                      return isMe ? (
                        <button
                          ...
                          className="w-full text-left rounded-xl px-4 py-3 transition-colors active:opacity-70"
                  ...
                  {!memoEditing && !myMemo && (
                    <button
                      ...
                      className="w-full py-2.5 rounded-xl text-sm border border-dashed text-gray-400 active:opacity-70"
          ...
          {/* 하단 버튼 */}
          <div className="px-5 pt-3 pb-4 flex-shrink-0 border-t border-gray-100 bg-white rounded-b-2xl flex gap-2">
            <button
              onClick={handleSubmit}
              disabled={loading || deleting}
              className="flex-1 py-3 rounded-xl text-white font-semibold text-sm transition-opacity active:opacity-80 disabled:cursor-not-allowed"
              style={{
                background: loading || deleting ? "#CCCCCC" : "#E85D2F",
              }}
            >
            ...
            {isEdit && canDelete && (
              <button
                ...
                className="flex-shrink-0 px-4 py-3 rounded-xl text-sm font-semibold border bg-white transition-opacity active:opacity-70 disabled:cursor-not-allowed"
                style={{
                  borderColor: deleting || loading ? "#D1D5DB" : "#111111",
                  color: deleting || loading ? "#9CA3AF" : "#111111",
                }}
              >
                {deleting ? "삭제 중..." : "삭제"}
              </button>
